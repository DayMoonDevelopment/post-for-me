  ...
  switch (action) {
      ...
      try {
        ...
        const updatedSchedule = await stripe.subscriptionSchedules.update(
          schedule.id,
          {
            ...
            metadata: {
              ...schedule.metadata,
              cancel_scheduled_by: currentUser.data.user.id,
        ...
        const endDate = updatedSchedule.phases?.[0]?.end_date
          ? new Date(
              updatedSchedule.phases[0].end_date * 1000,
            ).toLocaleDateString()
          : periodEnd
    ...
    case "create_checkout": {
      ...
      try {
        const checkoutSession = await stripe.checkout.sessions.create({
          ...
          customer_email: team.data.stripe_customer_id
            ? undefined
            : team.data.billing_email || undefined,
          mode: "subscription",
          line_items: [
            {
    ...
    default: {
      ...
      try {
        ...
        const mainProduct = await stripe.products.retrieve(
          STRIPE_API_PRODUCT_ID,
        );
