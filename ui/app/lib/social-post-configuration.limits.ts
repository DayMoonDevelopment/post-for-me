/**
 * Social Post Configuration — the LIMITS.
 *
 * The single source of truth for every per-platform number and enum a configuration
 * touches (title/description caps, media counts, poll rules). It exists so the UI
 * metamodel (`social-post-configuration.schema`) and the Zod validation
 * (`social-post-configuration.validation`) read the SAME values and can never drift —
 * YouTube's 100-char title and TikTok's 85-char title live here once.
 *
 * These are soft UI hints, approximate to each network's published limits. The Post for
 * Me API (and each platform) is the real source of truth on rejection; use these to
 * guide the UI, not to promise acceptance. Zero runtime dependencies.
 */
