/**
 * Social Post Configuration — the SCHEMA (UI metamodel).
 *
 * A declarative, renderer-agnostic description of every platform's configuration
 * fields. This is the foundational primitive: it is DATA, not UI. Every visual
 * treatment — accordion, tabs, drawer, inline — is just a renderer that walks
 * `SOCIAL_POST_CONFIGURATION_SCHEMA[provider]` and draws each field. Add a platform
 * capability here once and it appears in every treatment, with the right default,
 * grouping, and control.
 *
 * `control` is a SUGGESTED renderer, not a contract — a treatment may render an enum as
 * a segmented control, a select, or a pill row. The schema describes semantics (this is
 * an enum with these options, in this group, with this default); the block decides the
 * chrome.
 *
 * Field-level caps read from `social-post-configuration.limits` so the counter a UI
 * shows and the rule the validator enforces are the same number. `visibleWhen`
 * expresses conditional applicability the API SILENTLY IGNORES when unmet (e.g.
 * `audio_name` is meaningful only on reels) — it is a visibility hint, not a validation
 * rule. Rules the API REJECTS on live in `social-post-configuration.validation`.
 */
