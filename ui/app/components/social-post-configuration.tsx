  ...
  return (
    <AccordionItem value={platform}>
      ...
      {/* Aligns the panel content under the platform avatar in the header. The
          panel primitive already adds `px-2`, so `pl-6` on top of that lands the
          content just past the status dot + gap. (CLI mirrors `pl`/`pr` → `ps`/`pe`.) */}
      <AccordionContent className="grid gap-4 pl-6 pr-3">
        <ConfigFields
          fields={fields}
            ...
            {accounts.map((account, index) => {
              ...
              return (
                ...
                >
                  ...
                  {overridden ? (
                    // `ml-3` sits the left rule at the CENTER of the sm (size-6 = 24px)
                    // account avatar; `pl-5` lands the override content under the
                    // username (avatar + gap-2). (CLI mirrors to `ms-3`/`ps-5` for RTL.)
                    <div className="ml-3 border-l pl-5">
                      <ConfigFields
