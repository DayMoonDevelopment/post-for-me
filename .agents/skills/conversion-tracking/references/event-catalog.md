...
| Note | **One generic event for every outbound notification** — channel/category/type are properties, not distinct event names, so new emails *and* new channels (SMS/push) never proliferate events. Channel-specific details are namespaced by channel (`email_*`). The dispatcher (`process-team-notification.ts`) supplies `channel` + the namespaced props; the producer (`process-usage-limits.ts`) stamps `notification_category` (+ `threshold` on warnings) + a `tracking` block into `meta_data`. Only notifications with a `notification_category` emit this; uncategorized ones are skipped. Dedupe keys on channel + type + threshold + period + target tier, so a genuine escalation, a higher threshold crossing, or a second channel still counts while a retried cron pass collapses. |
