...
| Flag | `system_triggered: true` |
| properties (generic) | `channel` (`email`), `recipient_is_user` (whether the address mapped to a user), `notification_category` (`transactional`), `notification_type` (DB column — the communication bucket: `usage_alert` = informational threshold warning, `usage_limit_upgrade_notice` = an upgrade was scheduled on the team's behalf), `threshold` (crossed percent on warnings, e.g. `80`; `null` on the upgrade notice), `notification_id`, `usage_count`, `current_limit`, `plan_post_limit`, `suggested_plan_post_limit` |
