...
function determineProficiencyLevel(title: string, summary: string): string {
  ...
  if (content.includes('getting started') || content.includes('beginner') || content.includes('introduction') || content.includes('basics')) {
    return 'Beginner';
  }
  if (content.includes('advanced') || content.includes('expert') || content.includes('deep dive') || content.includes('optimization')) {
    return 'Advanced';
  }
  return 'Intermediate';
}
...
export const meta: Route.MetaFunction = ({ data }): MetaDescriptor[] => {
  ...
  const proficiencyLevel = determineProficiencyLevel(data.title, data.summary || '');
  ...
  const breadcrumbs = [
    ...
    { title: data.category?.name || "Category", href: `/resources/${data.category?.slug}` },
