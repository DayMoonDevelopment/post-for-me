  ...
  return (
    ...
    >
      <span className="truncate">
        {count === 0 ? (placeholder ?? t("playground.selectAccounts")) : `${count} selected`}
      </span>
...
export function AccountSelectorContent({
  ...
  return (
    ...
    >
      <Command>
        <CommandInput
          placeholder={searchPlaceholder ?? t("playground.searchAccounts")}
        />
        <CommandList>
          <CommandEmpty>{emptyText ?? t("playground.noAccountsFound")}</CommandEmpty>
          {groups.map((group) => (
            <CommandGroup
              key={group.platform}
