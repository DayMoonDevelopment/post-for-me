...
// ─── DASHBOARD DENSITY SKIN — re-apply after every ReUI merge ──────────────
// These are deliberately TIGHTER than ReUI's shipped defaults (which are
// `h-8`/`px-3` and `py-1.5`/`px-3 py-2`). The dashboard's tables are
// information-dense and pair with `text-xs/relaxed` typography, so upstream's
// roomier scale reads oversized here.
//
// The surrounding function shape is upstream's — only the class strings differ,
// so a future merge is a three-line reapplication rather than a rewrite. The
// rest of the skin lives in `data-grid-column-header.tsx` (typography),
// `data-grid-pagination.tsx` (typography + control sizing) and `data-grid.tsx`
// (`headerBackground`, container border).
const headerCellSpacingVariants = ({ size }: { size?: "dense" | "default" }) =>
  size === "dense" ? "px-2 h-7" : "px-2.5"
...
type DataGridTouchListLike = {
  item: (index: number) => { clientX: number; identifier: number; } | null
  length: number
}
  ...
  return (
    <>
      ...
      <Checkbox
        ...
        // `undefined`, not upstream's `null`: Base UI types `indeterminate` as
        // `boolean | undefined`. Same meaning, and it typechecks.
        indeterminate={
          row.getIsSomeSelected() ? !row.getIsSelected() : undefined
        }
...
function DataGridTableRowSelectAll() {
  ...
  return (
    <Checkbox
      ...
      // See the note in DataGridTableRowSelect — Base UI wants `undefined`.
      indeterminate={isSomeSelected ? !isAllSelected : undefined}
                  ...
                  .map((header) => {
                    ...
                    return (
                      <DataGridTableHeadRowCell header={header} key={header.id}>
                        ...
                        {props.tableLayout?.columnsResizable &&
                          column.getCanResize() ? <DataGridTableHeadRowCellResize header={header} /> : null}
...
function DataGridTable<TData>({
  ...
  return (
    <DataGridTableViewport>
      <DataGridTableBase>
        {renderHeader ? <DataGridTableHead>
            {mergedHeaderGroups.map((headerGroup) => {
              return (
                <DataGridTableHeadRow
                  key={headerGroup.id}
          ...
          </DataGridTableHead> : null}

        {renderHeader &&
          (props.tableLayout?.stripped || !props.tableLayout?.rowBorder) ? <DataGridTableRowSpacer /> : null}

        <DataGridTableBody>
          <MemoizedDataGridTableBodyRows table={table} />
        </DataGridTableBody>

        {footerContent ? <DataGridTableFoot>{footerContent}</DataGridTableFoot> : null}
      </DataGridTableBase>
    </DataGridTableViewport>
  )
}
