...
export type PostForMeIconProps = ComponentProps<"svg"> & {
  /**
   * "brand" (default) renders the official brand colors;
   * "monochrome" renders a single-color glyph inheriting `currentColor`.
   */
  variant?: "brand" | "monochrome";
};

/**
 * The Post for Me mark is single-color by design, so "brand" and
 * "monochrome" render identically (both inherit `currentColor`). The
 * variant prop exists for API consistency across brand icons.
 */
export function PostForMeIcon({
  variant = "brand",
  className,
  ...props
}: PostForMeIconProps) {
