...
export async function getXSocialProviderConnection({
  ...
  const { data: oauthData } = await supabaseServiceRole
    ...
    // Scoped to the project, matching the Bluesky provider. The caller already
    // resolved this row against the project, so today this is redundant — but
    // it's a service-role read (no RLS), and "the caller checked" is exactly
    // the assumption that stops being true when a caller is added.
