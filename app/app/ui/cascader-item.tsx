...
import { CheckIcon, ChevronRightIcon, LoadingIcon, MinusIcon, RepostIcon } from "~/icons";
...
const INDICATOR_CLASS =
  "pointer-events-none absolute flex items-center justify-center size-3.5 end-[var(--cascader-row-inset,8px)]! rtl:right-auto!"
...
const ROW_THEME_CLASS =
  "data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground rounded-md py-1 min-h-7 [&_svg:not([class*='size-'])]:size-3.5"
...
const CascaderItem = React.memo(function CascaderItem({
  ...
  if (isCascaderMoreNode(node)) {
    ...
    const moreBody = children ?? (
      <>
        {loading ? (
          <LoadingIcon className="size-3.5 animate-spin" aria-hidden />
        ) : null}
        <span>{text}</span>
...
const CASCADER_SEPARATOR_CLASS =
  "h-px my-1 bg-border/50 mx-[calc(var(--cascader-list-pad,4px)*-1)]! shrink-0"
