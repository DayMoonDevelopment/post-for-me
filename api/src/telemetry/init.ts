...
function initTelemetry(): void {
  ...
  const enabled = getBooleanEnv('OTEL_ENABLED', true);
  if (!enabled) {
    return;
  }
  ...
  const diagLevel = process.env.OTEL_DIAG_LOG_LEVEL?.trim().toUpperCase();
  if (diagLevel) {
  ...
  const serviceName = process.env.OTEL_SERVICE_NAME || 'post-for-me-api';
  const serviceVersion =
    process.env.OTEL_SERVICE_VERSION ||
    process.env.npm_package_version ||
    undefined;
  const environment = process.env.OTEL_ENVIRONMENT || process.env.NODE_ENV;
    ...
    instrumentations: [
      getNodeAutoInstrumentations({
        ...
        '@opentelemetry/instrumentation-fs': {
          enabled: getBooleanEnv('OTEL_INSTRUMENTATION_FS_ENABLED', false),
        },
      }),
    ],
  });
